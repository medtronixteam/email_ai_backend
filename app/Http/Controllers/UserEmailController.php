                ...
                if ($emails->count() > 0) {
                    ...
                    $emails->chunk($chunkSize)->each(function ($emailBatch, $batchIndex) use ($configData, $compains, $delayInterval) {
                        ...
                        foreach ($emailBatch as $email) {
                            ...
                            $jobBulk = BulkJob::dispatch($email->id, $configData->id, $compains->message, $compains->subject, $compains->attachments, $compains->id)->delay($delay);
                            $jobId = method_exists($jobBulk, 'getJobId') ? $jobBulk->getJobId() : null;
                            if ($jobId) {
                                Contact::find($email->id)->update(['job_id' => $jobId]);
                            }
